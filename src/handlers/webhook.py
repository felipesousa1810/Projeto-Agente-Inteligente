...
async def whatsapp_webhook(
    ...
    Recebe mensagens da Evolution API, processa com o agente,
    e envia resposta de volta.
    ...
    Returns:
        Dict com status, trace_id e intent.
    ...
    with tracer.start_as_current_span("whatsapp_webhook") as span:
        ...
        try:
            # 1. Verificar idempotência (fail open se Redis indisponível)
            try:
                ...
                # Redis unavailable - continue processing (fail open)
                logger.warning(
            ...
            # 4. Marcar como processado no Redis
            try:
                idempotency = get_idempotency_manager()
